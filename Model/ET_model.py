...
# model construction
model = ExtraTreesRegressor(n_estimators=100, max_depth=20, min_samples_leaf=1, min_samples_split=2)
...
print("ET testing results:")
print(f"RMSE: {rmse_test:.4f}")
print(f"MAE: {mae_test:.4f}")
print(f"R2 Score: {r2_test:.4f}")

# Save the model
model_filename = 'PWM_ET.joblib'
...
predictions_df = pd.DataFrame({
    ...
    'ET Predicted logKd': target_predictions
})

output_folder = r"ET_logKd_prediction"

if not os.path.exists(output_folder):
    os.makedirs(output_folder)

predictions_df.to_excel(os.path.join(output_folder, 'ET_Standard_AC_predictions.xlsx'), index=False)